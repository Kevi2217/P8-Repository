...
estimate_svar_irf <- function(sim_data,
                              ...
                              response_var = 1,
                              ci_level = 0.95,
                              n_boot = 200 # Adjust bootstrap runs as needed (more is better but slower)
) {
  
  # --- 1. Prepare Data ---
  # 'vars' package expects time series object or matrix with time in rows, variables in columns
  # Transpose the input data if necessary (assuming N rows x T cols input)
  ...
  # Use tryCatch to handle potential errors during VAR estimation (e.g., collinearity)
  var_model <- tryCatch({
  ...
  # vars::irf uses Cholesky decomposition by default when ortho=TRUE
  # It calculates IRFs for all impulse-response pairs.
  # We specify boot=TRUE for bootstrap CIs.
  irf_results <- tryCatch({
    vars::irf(var_model,
              impulse = impulse_var,   # Specify the impulse variable
              response = response_var, # Specify the response variable
              n.ahead = hmax,          # Maximum horizon
              ortho = TRUE,            # Use orthogonalized (structural) shocks via Cholesky
              ci = ci_level,           # Confidence level
              boot = TRUE,             # Perform bootstrap
              runs = n_boot,           # Number of bootstrap runs
              seed = NULL)             # Let seed be handled outside for replicability loop
  }, error = function(e) {
  ...
  # Ensure we extract correctly, accounting for the structure
  # irf_results$irf[[1]] accesses the point estimates for the specified impulse->response
  # irf_results$Lower[[1]] accesses the lower bounds
  # irf_results$Upper[[1]] accesses the upper bounds
...
analyze_svar_failure_boundary <- function(Ts, Ns, p_fixed = 4, rhos = c(0.2, 0.15, 0.1, 0.05), hmax = 10, estimate_function = estimate_svar_irf, ...) {
  ...
  for (t_val in Ts) {
    ...
    if (start_n_index <= n_count) {
      for (n_idx in start_n_index:n_count) {
        ...
        # --- Estimate SVAR IRF (Focus on Success/Failure) ---
      ...
      result_counter <- result_counter + 1
      failure_occurred_for_T <- TRUE # Mark as failed for consistency, although loop didn't run
    }
    ...
    if (!failure_occurred_for_T) {
      ...
      # We don't add a Status=0 here, as the boundary is *beyond* max_n_tested for this T
    }
  ...
  # This correctly handles the boundary point (Status=0) if it was recorded
...
plot_svar_failure_boundary <- function(boundary_data) {
  ...
  # --- Create Plot ---
  p <- ggplot() +
    # Plot successful points (optional, can be commented out for cleaner boundary plot)
    ...
    # Draw the line connecting the first failure points
    ...
    for(t in 1:length(current_Ts_for_N)){
      for(pi in 1:length(PIs)){
        ...
        for (h in 0:hmax) {
          ...
          # Handle potential NAs if some reps failed within the sim file
